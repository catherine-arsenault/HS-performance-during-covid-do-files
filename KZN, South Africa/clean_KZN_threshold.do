...
use "$user/$data/Data for analysis/KZN_Jan19-Dec20_WIDE_CCA_AN.dta"
	...
	u "$user/$data/Data for analysis/tmpanc1_util`m'.dta", clear
	...
	foreach x in totaldel del_util sb_mort_denom livebirths_denom cs_util pnc_util diarr_util pneum_util sam_util art_util opd_util ///
			   ...
			   trauma_util newborn_mort_num sb_mort_num mat_mort_num ipd_mort_num icu_mort_num trauma_mort_num {
				merge 1:1 Province dist subdist Facility factype using "$user/$data/Data for analysis/tmp`x'`m'.dta"
				drop _merge
				save "$user/$data/Data for analysis/KZN_Jan19-Dec20_WIDE_CCA_DB`m'.dta", replace
		}
	foreach x of global all {
