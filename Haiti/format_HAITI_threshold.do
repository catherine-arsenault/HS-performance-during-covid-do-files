...
u "$user/$data/Data for analysis/Haiti_Jan19-Dec20_WIDE_CCA_DB`m'.dta", clear
...
putexcel set "$user/$data/Analyses/Codebook for Haiti Internal.xlsx", sheet(T`m')  modify
...
putexcel set "$user/$data/Codebook for Haiti.xlsx", sheet(Final data `m')  modify
putexcel A2 = "Variable"
...
save "$user/$data/Data for analysis/Haiti_su_24months`m'.dta", replace 
}
