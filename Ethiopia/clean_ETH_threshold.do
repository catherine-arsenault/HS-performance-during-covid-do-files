...
use "$user/$data/Data for analysis/Ethiopia_Jan19-Dec20_WIDE_CCA_AN.dta"
	foreach x in  fp_util sti_util anc_util del_util cs_util pnc_util diarr_util pneum_util sam_util ///
			  ...
			  preserve
					keep  organisationunitname orgunitlevel1 orgunitlevel4 zone region `x'* 
	...
	preserve 
		* For these indicators we keep all woredas that report at least once.
		keep  organisationunitname orgunitlevel1 orgunitlevel4 zone region diab* hyper* 
		egen total = rowtotal(diab_util10_19-hyper_qual_num12_20), m 
		drop if total==.
		drop total 
		save "$user/$data/Data for analysis/tmpdiab_hyper`m'.dta", replace
	restore 
	
	u "$user/$data/Data for analysis/tmpfp_util`m'.dta", clear
		...
		* =33 indicators
		foreach x in sti_util anc_util del_util cs_util pnc_util diarr_util pneum_util sam_util ///
			 	...
			 	merge 1:1  organisationunitname orgunitlevel1 orgunitlevel4 zone region using "$user/$data/Data for analysis/tmp`x'`m'.dta", force 
				drop _merge
				save "$user/$data/Data for analysis/Ethiopia_Jan19-Dec20_WIDE_CCA_DB`m'.dta", replace
		}
