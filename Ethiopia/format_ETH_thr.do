...
* ALL INDICATORS
forvalues m=12 15 18 24 {
use "$user/$data/Data for analysis/Ethiopia_Jan19-Dec20_WIDE_CCA_DB`m'.dta", clear
...
putexcel set "$user/$data/Analyses/Codebook for Ethiopia Internal.xlsx", sheet(T `m')  modify
...
putexcel set "$user/$data/Codebook for Ethiopia.xlsx", sheet(Final data) modify 
putexcel A2 = "Variable"
...
***************************************************************************
merge 1:1  region zone org*  using "$user/$data/Data for analysis/Ethiopia_Jan19-Jun20_WIDE_CCA_TB`m'.dta"
drop _merge*/

save "$user/$data/Data for analysis/Ethiopia_Jan19-Dec20_CCA_DB_Complete`m'.dta", replace 
...
use "$user/$data/Data for analysis/Ethiopia_Jan19-Dec20_CCA_DB_Complete`m'.dta", clear 
...
reshape long diab_util hyper_util diab_qual_num hyper_qual_num fp_util sti_util anc_util ///
			  ...
			  newborn_mort_num sb_mort_num mat_mort_num er_mort_num  totaldel ///
			  tbnum_qual tbdenom_qual tbdetect_qual totalipd_mort_num diab_detec hyper_detec , ///
				i(region zone org* unique_id) j(month) string
...
drop month	
rename mo month
sort region zone org* unique_id year month 
...
save "$user/$data/Data for analysis/Ethiopia_su_24months`m'.dta", replace 
}
