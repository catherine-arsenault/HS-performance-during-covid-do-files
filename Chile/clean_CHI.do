...
u "$user/$data/Data for analysis/Chile_Jan19-Oct20_WIDE.dta", clear
...
drop all_visits 
********************************************************************
global volumes pnc_util cs_util del_util anc_util 
...
putexcel set "$user/$data/Codebook for Chile.xlsx", sheet(Tot fac reporting 22mos, replace)  modify
...
preserve
	local volumes pnc_util cs_util del_util anc_util
			   
	reshape long `volumes', i(org*) j(month, string)
	recode `volumes' (.=0) (1/999999999=1)
	collapse (sum) `volumes', by(month)
	putexcel set "$user/$data/Codebook for Chile.xlsx", sheet(MinMax fac reporting 22mos, replace)  modify
...
save "$user/$data/Data for analysis/Chile_Jan19-Oct20_WIDE_CCA_AN.dta", replace 
