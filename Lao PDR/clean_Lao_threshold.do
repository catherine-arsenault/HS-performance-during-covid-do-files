...
putexcel set "$user/$data/Codebook for Lao PDR.xlsx", sheet(Raw data)  modify
putexcel A2 = "Variable"
...
use "$user/$data/Data for analysis/Lao_Jan19-Dec20_WIDE_CCA_AN.dta", clear
	...
	u "$user/$data/Data for analysis/tmpfp_perm_util`m'.dta", clear
	* All variables except the first 
	foreach x in  fp_sa_util fp_la_util anc_util del_util cs_util ///
			   ...
			   neo_mort_num sb_mort_num mat_mort_num {
			 	merge 1:1 org* using "$user/$data/Data for analysis/tmp`x'`m'.dta"
				drop _merge
				save "$user/$data/Data for analysis/Lao_Jan19-Dec20_WIDE_CCA_DB`m'.dta", replace
		}
	foreach x of global all {
