...
use "$user/$data/Data for analysis/Lao_Jan19-Dec20_WIDE_CCA_DB`m'.dta", clear
...
putexcel set "$user/$data/Analyses/Codebook for Lao PDR Internal.xlsx", sheet(T`m', replace)  modify
...
preserve
	...
	putexcel set "$user/$data/Analyses/Codebook for Lao PDR Internal.xlsx", sheet(T`m')  modify
...
putexcel set "$user/$data/Codebook for Lao PDR.xlsx", sheet(Final data `m')  modify
putexcel A2 = "Variable"
...
save "$user/$data/Data for analysis/Lao_su_24months`m'.dta", replace
}
