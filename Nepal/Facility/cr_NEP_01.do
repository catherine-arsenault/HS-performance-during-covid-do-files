...
* Created by Byron Cohen and MK Kim 
* HS performance during Covid
* Sep 10, 2020 
* Nepal, January 2019 - June 2020
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~		
	* ANC 2019
	...
	egen anc_util1_19 = rowtotal( antenatalcheckupfirstancvisitany v20), m
	egen anc_util2_19 =	rowtotal( v9 v21), m
	egen anc_util3_19	=rowtotal( v10 v22), m
	egen anc_util4_19	=rowtotal( v11 v23), m
	egen anc_util5_19	=rowtotal(v12 v24), m
	egen anc_util6_19=	rowtotal(v13  v25), m
	egen anc_util7_19	=rowtotal( v14 v26 ), m
	egen anc_util8_19=	rowtotal(v15  v27), m
	egen anc_util9_19=	rowtotal(v16 v28), m
	egen anc_util10_19	=rowtotal(v17 v29 ), m
	egen anc_util11_19	=rowtotal(v18 v30 ), m
	egen anc_util12_19 =rowtotal(v19  v31), m
	keep org* anc* 
	duplicates tag org* , gen(tag) // there are 2 duplicate facilities, drop those with no ANC data 
	...
	* ANC 2020
	import delimited "$user/$data/Raw data/Nepal_2020_Jan-June_facility_anc_util", encoding(UTF-8) clear 
	egen anc_util1_20 = rowtotal(antenatalcheckupfirstancvisitany v14), m
	egen anc_util2_20 =	rowtotal(v9 v15 ), m
	egen anc_util3_20	=rowtotal(v10 v16), m
	egen anc_util4_20	=rowtotal(v11  v17 ), m
	egen anc_util5_20	=rowtotal(v12 v18), m
	egen anc_util6_20=	rowtotal(v13 v19 ), m
	keep org* anc* 
	duplicates tag org* , gen(tag)
	egen total= rowtotal(anc*), m
	drop if tag==1 & total==.
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	* Facility Deliveries 2019
	...
	rename (deliveryserviceskilledbirthatten-v22) ///
	...
	* Facility Deliveries 2020
	...
	rename (deliveryserviceskilledbirthatten-v16) (del_util1_20 del_util2_20 del_util3_20 del_util4_20 ///
...
********************************************************************************
	...
	* BCG 2019
	...
	egen bcg_util1_19 = rowtotal(immunizationprogramvaccinetypech), m
	egen bcg_util2_19 =	rowtotal(v12), m
	egen bcg_util3_19 = rowtotal(v13), m
	egen bcg_util4_19 = rowtotal(v14), m
	egen bcg_util5_19 = rowtotal(v15), m
	egen bcg_util6_19 = rowtotal(v16), m
	egen bcg_util7_19 = rowtotal(v17 ), m
	egen bcg_util8_19 = rowtotal(v18), m
	egen bcg_util9_19 = rowtotal(v19), m
	egen bcg_util10_19 = rowtotal(v20 ), m
	egen bcg_util11_19 = rowtotal(v21 ), m
	egen bcg_util12_19 = rowtotal(v22), m
	keep org* bcg* 
	duplicates tag org* , gen(tag) // there are X duplicate facilities, drop those with no bcg data 
	egen total= rowtotal(bcg*), m
	drop if tag==1 & total==.
	drop tag total
	merge 1:1 orgunitlevel1	orgunitlevel2	orgunitlevel3	orgunitlevel4	orgunitlevel5	orgunitlevel6 organisationunitid	organisationunitname	organisationunitcode using "$user/$data/Data for analysis/Nepal_Jan19-Jun20_WIDE.dta"
	drop _merge
	save "$user/$data/Data for analysis/Nepal_Jan19-Jun20_WIDE.dta", replace

	* BCG 2020
	import delimited "$user/$data/Raw data/Nepal_2020_Jan-June_facility_bcg_qual.csv", encoding(UTF-8) clear 
	egen bcg_util1_20 = rowtotal(immunizationprogramvaccinetypech), m
	egen bcg_util2_20 =	rowtotal(v12 ), m
	egen bcg_util3_20	=rowtotal(v13), m
	egen bcg_util4_20	=rowtotal(v14), m
	egen bcg_util5_20	=rowtotal(v15), m
	egen bcg_util6_20=	rowtotal(v16 ), m
	keep org* bcg* 
	duplicates tag org* , gen(tag)
	egen total= rowtotal(bcg*), m
	drop if tag==1 & total==.
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	*Measles Vaccines 2019
	...
	egen measles_util1_19 = rowtotal( immunizationprogramchildrenimmun v23), m
	egen measles_util2_19 =	rowtotal( v12 v24 ), m
	egen measles_util3_19 = rowtotal( v13 v25 ), m
	egen measles_util4_19 = rowtotal( v14 v26 ), m
	egen measles_util5_19 = rowtotal(v15 v27 ), m
	egen measles_util6_19 = rowtotal(v16 v28 ), m
	egen measles_util7_19 = rowtotal( v17 v29  ), m
	egen measles_util8_19 = rowtotal(v18 v30 ), m
	egen measles_util9_19 = rowtotal(v19 v31 ), m
	egen measles_util10_19 = rowtotal(v20 v32 ), m
	egen measles_util11_19 = rowtotal(v21 v33  ), m
	egen measles_util12_19 = rowtotal(v22 v34 ), m
	keep org* measles* 
	...
	*Measles Vaccines 2020
	...
	egen measles_util1_20 = rowtotal(immunizationprogramchildrenimmun v17 ), m
	egen measles_util2_20 = rowtotal(v12 v18 ), m
	egen measles_util3_20 = rowtotal(v13 v19 ), m
	egen measles_util4_20 = rowtotal(v14 v20 ), m
	egen measles_util5_20 = rowtotal(v15 v21 ), m
	egen measles_util6_20 = rowtotal(v16 v22 ), m
	keep org* measles* 
	duplicates tag org* , gen(tag)
	egen total= rowtotal(measles*), m
	drop if tag==1 & total==.
...
********************************************************************************
	*TB cases detected 2019
	...
	rename disaggregationbysexcasteethnicit v11
	egen tb_detect1_19 = rowtotal( v11), m
	egen tb_detect2_19 = rowtotal( v12), m
	egen tb_detect3_19 = rowtotal( v13), m
	egen tb_detect4_19 = rowtotal( v14), m
	egen tb_detect5_19 = rowtotal(v15), m
	egen tb_detect6_19 = rowtotal(v16), m
	egen tb_detect7_19 = rowtotal( v17), m
	egen tb_detect8_19 = rowtotal(v18), m
	egen tb_detect9_19 = rowtotal(v19), m
	egen tb_detect10_19 = rowtotal(v20), m
	egen tb_detect11_19 = rowtotal(v21), m
	egen tb_detect12_19 = rowtotal(v22), m
	keep org* tb_detect* 
	duplicates tag org* , gen(tag) 
	egen total= rowtotal(tb_detect*), m
	drop if tag==1 & total==.
	drop tag total
	merge 1:1 orgunitlevel1	orgunitlevel2	orgunitlevel3	orgunitlevel4	orgunitlevel5	orgunitlevel6 organisationunitid	organisationunitname	organisationunitcode using "$user/$data/Data for analysis/Nepal_Jan19-Jun20_WIDE.dta"
	drop _merge
	...
	*TB cases detected Jan-Jun 2020
	...
	rename disaggregationbysexcasteethnicit v11
	egen tb_detect1_20 = rowtotal(v11 ), m
	egen tb_detect2_20 = rowtotal(v12), m
	egen tb_detect3_20 = rowtotal(v13 ), m
	egen tb_detect4_20 = rowtotal(v14 ), m
	egen tb_detect5_20 = rowtotal(v15 ), m
	egen tb_detect6_20 = rowtotal(v16 ), m
	keep org* tb_detect* 
	duplicates tag org* , gen(tag)
	egen total= rowtotal(tb_detect*), m
	drop if tag==1 & total==.
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
	*HIV cases diagnosed 2019
	...
	rename disaggregationbysexcasteethnicit v11
	egen hivdiag1_19 = rowtotal( v11 v23), m
	egen hivdiag2_19 = rowtotal( v12 v24), m
	egen hivdiag3_19 = rowtotal( v13 v25), m
	egen hivdiag4_19 = rowtotal( v14 v26), m
	egen hivdiag5_19 = rowtotal(v15 v27), m
	egen hivdiag6_19 = rowtotal(v16 v28), m
	egen hivdiag7_19 = rowtotal( v17 v29), m
	egen hivdiag8_19 = rowtotal(v18 v30), m
	egen hivdiag9_19 = rowtotal(v19 v31), m
	egen hivdiag10_19 = rowtotal(v20 v32), m
	egen hivdiag11_19 = rowtotal(v21 v33), m
	egen hivdiag12_19 = rowtotal(v22 v34), m
	keep org* hivdiag* 
	duplicates tag org* , gen(tag) 
	...
	*HIV cases diagnosed Jan-Jun 2020
	...
	rename disaggregationbysexcasteethnicit v11
	egen hivdiag1_20 = rowtotal(v11 v17), m
	egen hivdiag2_20 = rowtotal(v12 v18), m
	egen hivdiag3_20 = rowtotal(v13 v19), m
	egen hivdiag4_20 = rowtotal(v14 v20), m
	egen hivdiag5_20 = rowtotal(v15 v21), m
	egen hivdiag6_20 = rowtotal(v16 v22), m
	keep org* hivdiag* 
	duplicates tag org* , gen(tag)
	egen total= rowtotal(hivdiag*), m
	drop if tag==1 & total==.
