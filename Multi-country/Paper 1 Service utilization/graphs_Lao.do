...
/* Vars needed for ITS (we expect both a change in level and in slope: 
	...
	seasons to adjust for seasonality */ 
gen rmonth= month if year==2019
replace rmonth = month+12 if year ==2020
sort reg rmonth
gen postCovid = rmonth>15 // pandemic period is months 16 to 24
...
putexcel set "$analysis/Results/Tables/Results MAY28.xlsx", sheet(Lao)  modify
putexcel A1 = "Lao PDR Region OLS FE"
...
* Deliveries
			...
			graphregion(color(white)) title("Lao PDR deliveries", size(small)) ///
...
* Outpatient
			...
			graphregion(color(white)) title("Lao PDR outpatient visits", size(small)) ///
