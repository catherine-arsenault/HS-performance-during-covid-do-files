...
foreach c in CHL ETH GHA HTI KZN LAO MEX NEP KOR THA {
	...
	putexcel set "$analysis/Results/Tables/Results by country OCT12.xlsx", sheet("`c'")  modify
...
* 	Heatmap
********************************************************************************
	import excel using "$analysis/Results/Tables/Relative % resumption OCT12.xlsx", sheet(CHL) firstrow clear
	save "$analysis/Results/Tables/tmp.dta", replace
	foreach c in  ETH GHA HTI KZN LAO MEX NEP KOR THA {
		import excel using "$analysis/Results/Tables/Relative % resumption OCT12.xlsx", sheet(`c') firstrow clear
		append using "$analysis/Results/Tables/tmp.dta"
		save "$analysis/Results/Tables/tmp.dta", replace
	}
	rename F country
