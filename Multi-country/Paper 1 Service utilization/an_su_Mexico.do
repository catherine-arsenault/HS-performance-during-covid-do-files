...
putexcel set "$analysis/Results/Tables/Results MAY28.xlsx", sheet(Mexico)  modify
putexcel A1 = "Mexico Delegation OLS FE"
...
* Deliveries
			...
			graphregion(color(white)) title("Mexico, IMSS deliveries", size(small)) ///
...
* Outpatient
			...
			xtitle("", size(small)) legend(off) ///
			ytitle("Average number per delegation", size(vsmall)) ///
			graphregion(color(white)) title("Mexico, IMSS outpatient visits", size(small)) ///
